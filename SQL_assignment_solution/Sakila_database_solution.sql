...
-- Q6 Display the first and last name of each actor in a single column in upper case letters. Name the column Actor Name. (200 rows) 
     SELECT CONCAT(UPPER(first_name), ' ', UPPER(last_name)) AS full_name FROM actor AS full_name;
...
-- Modify the above query to display from row # 16 to 20 with all columns. (5 rows) 
...
-- Q10 What is the value of name and population in the rows with ID =5, 23, 432 and 2021. Pl. write a single query to display the same. (4 rows). 
  SELECT Name,Population,ID FROM city WHERE ID IN (5, 23, 432, 2021);
...
-- Q1 Which actor has appeared in the most films? (‘107', 'GINA', 'DEGENERES', '42') 
   SELECT actor.actor_id, actor.first_name, actor.last_name, COUNT(film_actor.actor_id) as film_count FROM film_actor
...
-- Q2 What is the average length of films by category? (16 rows) 
        ...
        JOIN  film_category ON  film.film_id = film_category.film_id GROUP BY film_category.category_id;
